...
#   P1-CLASH-HUEVO-Y-VIVERES · 2026-09-09 — el vocabulario del autofix fruta-dulce/base-salada listaba las PREPARACIONES del huevo (`revoltillo`, `revuelto`) y no el INGREDIENTE: «huevo+mango», el primer ejemplo canónico de su propio docstring, no se detectaba y llegó al plan vivo 0871ea93. Añadidos `huevo` y las bases criollas (`mangu`, `platano verde`, `tostones`, `mofongo`); NO `platano` a secas ni `casabe` — la frontera de falsos positivos va medida en el test.
#   P1-CANDIDATO-CON-PRECIO · 2026-09-09 — el precio entra en la ELECCIÓN del plato, no sólo en la factura: `effective["budget"]["tier"]` existía desde la Fase 2 y `horizon.py` —quien elige— no nombraba `budget` ni una vez. `dish_cost` costea la ración en crudo desde `master_ingredients` y `template_candidates(budget_tier=...)` recorta la cola cara. Medido con precios reales: el peor día posible con presupuesto `low` cae de RD$585 a RD$247 (30 d: 17.565 → 7.409, suelo 13.650) y salen res, camarones, chivo y bistec; con `high` se expulsan 0. Sin precio ⇒ `None`, nunca suma parcial (subestimar ASCIENDE lo que no se puede costear); la poda es relativa y jamás vacía.
#   P1-DIA-DETERMINISTA-VARIEDAD · 2026-09-09 — antes de encender el día determinista le armé al dueño sus 14 días reales: 14/14, 0 comidas sucias, calorías +0,0 %… y **7 platos distintos en 56 comidas**, el mismo almuerzo los 14 días. `template_candidates` rotaba la lista y `elegir_plantilla` la recorría entera devolviendo `cands[0]`: la rotación era INERTE. Ahora `elegir_plantillas` devuelve la ventana de empate (medida: +0,50 da 4-8 elegibles por franja; +0,05 daba UNO), rotada con desfase POR FRANJA (sha256, no `hash()`, que está salado por proceso) y sin repetir plantilla dentro del mismo día. Además, un plato sin receta congelada ya no tira el DÍA ENTERO al LLM: se prueba el siguiente. Medido: 7 → 22 platos, peor repetición 9× → 5×, determinismo intacto (dos corridas, hash idéntico).
#   P1-CATALOGO-PROTEINA-DESAYUNO · 2026-09-09 — el desayuno criollo y la merienda son carbohidrato casi puro (mediana 3,86 y 3,37 g de proteína/100 kcal contra un objetivo de 5,86; sólo 7 de 45 y 6 de 40 llegaban al 85 %), y por eso 14 días deterministas daban 22 platos distintos. Ensanchar la ventana NO era la vía: medido, llegar a 44 platos así cuesta 9 de 14 días bajo el piso clínico. +20 platos construidos sobre esa misma cocina (claras, atún, sardinas, hígado, tilapia, cottage, jamón de pavo), cinco puertas cada uno y receta a mano contra las quince reglas. Destapó DOS defectos míos: la ventana `mejor + margen` se estrecha sola —añadir un plato bueno EXPULSA a otros— y `verifica_comida` corría FUERA del bucle, así que un plato sucio tiraba el DÍA ENTERO (14/14 → 12/14 justo al haber más donde elegir). Resultado: 22 → 35 platos, 14/14 días, 0 sucias, y 0 de 14 días bajo el piso de proteína (antes 4).
#   P1-RETINOL-PREFORMADO · 2026-09-09 — al medir la variedad sobre el ciclo REAL de 30 días (había medido 14) salió que un plato que metí esta misma mañana se servía 8 veces al mes: hígado encebollado, 120 g = 5.964 mcg RAE = **2× el UL diario de vitamina A**, y ni el escáner culinario ni el backstop clínico lo veían. El techo YA existía (`_MICRO_CLOSER_UL["vit_a_mcg"]=3000`) y sólo miraba a quien SUBE —impide que el cerrador de micros escale hígado— así que un plato que nace por encima pasaba por debajo. La distinción que hace correcto el guard: el UL es de retinol **preformado**, y de 18 días vivos que pasaban el UL los 2 eran **auyama** (beta-caroteno, no intoxica); un techo sobre RAE total habría roto cocina dominicana legítima. El plato se RETIRÓ en vez de encogerlo: ~60 g ya agotan el techo del día y el selector no tiene memoria entre días. Medido tras retirarlo: 30/30 días, 49 platos distintos (subió), 0 comidas sucias.
#   P1-DESC-CATEGORIA-FALSA · 2026-09-09 — «Desayuno dominicano de tubérculo» sobre un plato de PLÁTANO, que es el fruto de una musácea; en RD entra en «víveres», que es una categoría CULINARIA y mezcla tubérculos de verdad (yautía, ñame, batata, yuca) con los que no lo son. Medido: 1 de 72 fichas vivas, así que el arreglo es una lima y no una taxonomía — vigila UNA afirmación y sólo la BORRA cuando el plato no lleva ninguno (el casabe cuenta: se hace de yuca). Determinista y no una regla más del prompt, porque hoy quedó medido dos veces que pedir no es imponer. Va en el escudo pre-INSERT, el único chokepoint que corre para todo plan.
#   P1-MINUTOS-DE-LA-RECETA · 2026-09-10 — tercera ronda de juicio humano: 19 de 19 «cambiar», y al clasificar las notas 13 de ellas pedían corregir el TIEMPO. «Yogurt griego con guineo» —pelar un guineo— anunciaba 30 minutos. El número salía de una tabla de 6 filas por TÉCNICA con 30 de defecto, y medido: 21 de 198 plantillas tenían una técnica que no casaba ninguna fila (16 de ellas `crudo`) y se llevaban el defecto en silencio. La receta escrita YA declara sus tiempos paso a paso: `minutos_de_los_pasos` los suma, toma el TOPE de cada rango y descuenta lo que corre dentro de otro paso («Mientras los garbanzos cuecen…»), que fue la trampa que infló «Víveres guisados» a 125 min en mi primera versión. Acierta el número que pidió el dueño en 13 de 13 (±5 min); 177 plantillas mueven su tiempo y `prep_minutes_source` dice de dónde sale cada uno. La sonda que lo validó tenía su PROPIA copia del parser: hasta apuntarla a la función que se despliega, el 13/13 no medía lo desplegado.
#   P1-SODIO-DEL-DIA-DETERMINISTA · 2026-09-10 — 8 de las 19 notas de la tercera ronda pedían bajar la sal «y recalcular sodio»; su razón es de CONTABILIDAD, no de sabor: la sal que se va con el agua del hervor no se come. Al medirlo salió el agujero de debajo: `SODIUM_DAY_CEILING_MG` (2.000 mg, OMS) y su autofix viven en `assemble_plan_node`, y un día del módulo determinista NO pasa por ahí — la misma clase de superficie que cerró P0-DEGRADED-SAFETY-SCAN y la misma que dejó pasar el hígado. Medido sobre los 30 días REALES del dueño con su canario encendido: 13 de 30 días por encima del techo, máximo 3.867 mg. Dos capas: (1) la Sal declarada se acota a 0,5 g por ración —64 líneas; 34 plantillas declaraban 2 g y una 3,5— y eso deja 1 de 30; (2) un juicio de sodio al cerrar el día, con el candidato salado guardado como RESERVA en vez de descartado (descartar platos sanos por una cuenta acumulada castiga al último slot). Resultado: 0 de 29 días sobre el techo, máximo 1.933 mg, y el único día que no cabe —lo empuja el arenque, salado de origen— cae al LLM, que sí pasa por el autofix.
#   P1-RECETA-JUZGADA-R3 · 2026-09-10 — reescritas las 19 recetas con las correcciones que el dueño escribió en la tercera ronda. La de SEGURIDAD: «Tortilla de yuca con atún» rallaba la yuca CRUDA, la exprimía y la freía, y él escribió «no confiar en rallar y dorar para eliminar sus compuestos tóxicos». Tenía razón: los glucósidos cianogénicos de la yuca los arrastra HERVIR y BOTAR el agua. Además: la sal entra después de escurrir en los platos que botan el agua del hervor (la que se va con el agua no se come y la ficha la contaba igual), el mangú gana `Vinagre blanco` como constituyente —la cantidad va en la LISTA, no en el paso, que es la regla de sus rondas anteriores— y la tilapia declara que hornea «mientras la yuca hierve», lo que baja su tiempo de 45 a 30 min sin tocar un número porque P1-MINUTOS-DE-LA-RECETA descuenta el solape. Medido tras reescribir: 14/14 días armados, 0 comidas sucias de 56, calorías −0,1 %, 34 platos distintos, RD$4.022 el ciclo.
#   P1-DO-PLATO-SIN-MERCADO · 2026-09-10 — la decisión que el dueño delegó («haz lo que mejor consideres»): de los tres platos no dominicanos sale el de FRIJOL CARGAMANTO y se quedan habas y guisantes secos. Tres razones medidas: es colombiano (vive ya en la biblioteca de Colombia), su fila del catálogo está VACÍA para RD —precio 0, sin peso—, así que ni la lista de compras ni el filtro de presupuesto lo ven, y su papel lo cubre «Habichuelas rojas guisadas con auyama, arroz y aguacate». Habas (RD$205/lb) y guisantes (RD$133) son caros, pero con precio: el filtro los gestiona sin borrar nada. Al medirlo salió lo de al lado: los platos que no se pueden costear son el 10,6 % de la biblioteca y el 19,8 % de lo que se le sirve —«lo sin precio sobrevive siempre» es correcto, pero al podar el 40 % caro de lo que SÍ tiene precio lo que no lo tiene gana cuota— y la causa son 8 productos dominicanos ausentes de `supermarket_products` (queso de papa en 8 platos). Esos precios son del dueño: aquí no se inventan. Mi primera lectura decía «134 filas sin precio, 38 %»: son las filas beta de los otros países, sin precio A PROPÓSITO.
#   P1-CATALOGO-DICE-QUE-PRODUCTO · 2026-09-10 — nueve de las 19 notas de la tercera ronda pedían «especificar el porcentaje de grasa» y «verificar según la etiqueta». Verificado contra USDA FoodData Central y contra SU `supermarket_products`: el Queso cottage llevaba el 1 % (fdc 173417) y su supermercado sólo vende 2 % y 4 % — los números eran de un producto que no puede comprar ⇒ la fila ENTERA pasa a 2 % (fdc 172182; una fila mitad 1 % y mitad 2 % sería peor que cualquiera). El Tofu firme tenía valores de tofu FIRME con el puntero del tofu NORMAL (172476, 76 kcal) ⇒ sólo el puntero, a 172475; tocar un número habría sido inventar un defecto. Salsa de soya (sus números son de soya REDUCIDA en sodio, y su súper vende Kikkoman Baja en Sal y La Choy Lite) y Yogurt griego sin azúcar (griego natural 0 %) ganan `gloss_es` con el producto que describen; `name` no se toca. La migración se probó dentro de una transacción con ROLLBACK antes de aplicarse. Queso de hoja queda fuera y dicho: valores manuales, sin referencia USDA. Y una consecuencia mía, medida antes del gate: con el cottage real, «Queso cottage con casabe, tomate y aguacate» cayó de 5,87 a 4,79 g de proteína/100 kcal, por debajo del 5,0 con el que entró — reequilibrado (casabe 55→40 g, cottage 170→200 g) a 5,90 en vez de esconderlo.
#   P1-QUESO-DE-PAPA-A-GOUDA · 2026-09-10 — el dueño preguntó si el «queso de papa» podía ser cualquier queso. Medido: la fila es EDAM (USDA 173419) con un nombre PUERTORRIQUEÑO —el catálogo la tiene en el léxico de PR, sin precio dominicano—, así que los 8 platos dominicanos que la usaban no se podían costear y, por la regla «lo sin precio sobrevive siempre», se servían de más. El gouda es prácticamente el mismo queso (356 kcal · 24,9 g prot · 27,4 g grasa contra 357 · 25,0 · 28,6) con un 16 % menos de sodio, y su súper vende 17 goudas a RD$320/lb. Se cambia el ingrediente Y el nombre del plato —con el nombre en «queso de papa» y la lista en «Queso gouda», iría a buscar un queso que su mercado no tiene—, y las 8 recetas se re-clavan con `plan_policy.mint_template_id`, no con una copia. De paso: el puntero USDA del gouda era el del FETA (173420) ⇒ 171241 sin tocar un valor, y 3 de 193 recetas dominicanas estaban en voseo («Rallá», «Comélo») ⇒ tuteo.
#   P1-DIA-DETERMINISTA-VARIEDAD-DEL-DIA · 2026-09-10 — al pasar el «queso de papa» a gouda salieron a la vista dos platos que eran el mismo: «Arepa de maíz con queso gouda» y «Arepitas de maíz con queso gouda (horneadas)», 11 de 120 comidas y un día con los dos. El camino del modelo tiene DOS puertas para eso —`_SAME_DAY_PROTEIN_GATE_LABELS` (carnes, pescados y huevo; exime queso, legumbres y yogur, que en RD se repiten por cultura) y `_LIGHT_BASE_TOKENS` (la misma base en desayuno Y merienda)— y el día determinista no pasa por ellas: la CUARTA vez hoy con la misma forma, tras el retinol, el sodio y el path degradado. Se leen de donde viven; el permiso de repetir proteína sale de `horizon.repetition_limits_for` (su plan está en `balanced`: no se repite). Se PREFIERE, no se descarta —el patrón del sodio—. Pero se despliega APAGADA, por un A/B con el mismo instrumento: encendida, los días con proteína repetida van de 12 a 0 y los de base de 4 a 0, y a cambio las semanas que rompen el tope `balanced` de 2 repeticiones exactas pasan de 9 a 28 — el módulo no tiene memoria entre días y restringir el día concentra la repetición en los platos exentos. Ese 9 con la puerta apagada es el hallazgo de verdad: el día determinista ya rompe el tope semanal del dueño. Lo que sí queda activo: `bases_ligeras.py` extraído del god-file (en su tope de 53.100) con la familia «arepita»→«arepa», que la autocrítica del modelo tampoco veía, y el arreglo de mi reserva de sodio de esta mañana, que sólo guardaba al PRIMER candidato salado y aceptaba al segundo de largo.
#   P1-DO-DESPENSA-DE-SU-MERCADO · 2026-09-10 — los 12 platos dominicanos que seguían sin precio colgaban de 6 productos y el dueño los resolvió con capturas de su supermercado y lo que sabe de su cocina: el azúcar morena es «azúcar crema» (Wala) y el pan rallado «pan molido» (Buenhorno), y el Sazón Goya culantro y achiote y el hummus se venden ⇒ precio `owner_verified` y el nombre dominicano en `gloss_es` (la identidad no se renombra); el sofrito se HACE ⇒ desglosado en ají cubanela, cilantro, cebolla y ajo; la sémola de maíz no la conoce ⇒ fuera los 3 platos (2 chenchén y el chacá); el queso de hoja va en funditas sin etiqueta ⇒ se documenta la procedencia de sus valores en vez de inventar un puntero. Medido: 0 de 194 platos sin precio (eran 12) y 0 % de lo servido. Tres defectos que destapó la promoción: el token SUELTO `achiote` de México reclamaba «Sazón con culantro y ACHIOTE» ya con precio (`_COUNTRY_CATALOG_PRICED_SHADOWS`); el bloque de RD quedó vacío y un `()` falsy caía a la UNIÓN de los seis países (el generador habría verificado jamón serrano para un dominicano); y los cuatro salían A GRANEL en la lista —«¼ lb de sazón» a RD$280,66, el pan molido en «5 Uds.» sin coste porque la palabra «pan» le prestaba el peso de un pan— ⇒ los envases de sus capturas: caja de 8 sobres RD$99, funda de 2 lb RD$71, paquete de 1 lb RD$73 y pote de 10 oz RD$299.
#   P1-AVISO-CAPADO-LEE-EL-ENVASE · 2026-09-11 — «1 caja (8 sobres · 1,41 oz) · alcanza ~5 de 7 días — recompra» sobre una caja de sazón de 40 g que cubre la semana. Medido: la necesidad llega en 40 g, P6-SPICE-CAP la recorta a 28 g y la lista compra 40 g; el número salía de post/pre (28/40), la fracción que sobrevive al TOPE y no la que cubre lo COMPRADO. P1-COVERAGE-VS-PURCHASE ya tenía la regla (`_purchase_covers_need`), pero la evaluaba ANTES de que el ítem llevara `package_grams` (P1-BRAND-SIZE-FILTER lo adjunta 30 líneas después): con cualquier envase caía en «ante la duda, False» y sólo funcionaba con lb/kg/g — sus tests pasaban `package_grams` a mano. Barrido del catálogo: 6 de las 7 especias en envase bajo el tope avisaban de recompra con un envase que cubre la semana, y con 100 g de necesidad el número mentía a la baja (pimentón «~2 de 7» con un frasco de 85 g que cubre ~6). Ahora lo comprado en gramos se compara con la necesidad ANTES del tope —calla si cubre, y si no los días salen de lo comprado— sólo cuando el tope habla en gramos (su `post` es lo que llega); si registró sobres, o el envase se recontó por unidades, queda la fracción del tope. La compra no cambia. Visto al medir y NO tocado: «1 sobre» de sazón entra como 40 g porque la unidad de envase se convierte con el peso de la CAJA, no con los 5 g del sobre.
#   P1-AUDITORIA-ARQ-VERIFICADA · 2026-09-11 — verificación independiente de los 16 hallazgos de `BioBoros_auditoria_arquitectura.md` (revisión auditada baf4163; el informe y sus 14 sondas NO estaban en disco: se contrastaron los enunciados contra el código). Resultado: 11 vigentes, 3 parciales, 1 ya corregido (el 16, en 98f0bda), 0 falsos positivos. Lo corregido en esta pasada, con test por hallazgo (`test_p1_auditoria_arq_verificada.py`): el día determinista leía `slots`/`protein` (forma del blueprint) cuando el único llamador le pasa un `DaySkeletonModel` (`meal_types`/`protein_pool`) ⇒ siempre 4 comidas y sin familia — ahora franjas, etiquetas y reparto salen de los SSOT del camino del LLM; se le pasan al registry los filtros que `horizon` ya pasaba (mercado, nutrientes exigidos, durabilidad), el CandidateSet fijado al run y la cocina del día; `_no_escala` casaba «sal» dentro de Salami/Salsa/Ensalada (15 plantillas con un constituyente congelado) ⇒ frontera de palabra; el sodio del día se cargaba con la ración base del registry, no con el plato escalado (hasta 1,6×) ⇒ línea a línea contra el catálogo, con «sin dato» dicho; el backstop clínico era fail-open en este camino ⇒ fail-secure como en los otros dos; la receta congelada copiaba «tres tazas de agua» para 54 g de quinoa ⇒ el agua medida escala con el factor; el tiempo del plato sale del registry y `assemble_plan_node` deja de inventar «15 min» (vacío + fuente dicha); `/recipe/expand` ya no reescribe una receta firmada en 3 pasos; el gate culinario en `block` moría por un `:.0%` sobre `None` dentro del try; el guard de coherencia marca el plan cuando NO pudo evaluar en vez de devolver «sin divergencias»; y seis mutaciones (`shift-plan`, `grocery-start-date`, `restore`, `restore-local`, `restock`, `inventory/consume`) re-encolan la proyección de compras, que además ahora lleva la huella de la Nevera. Premisa corregida en cuatro docstrings: el día determinista SÍ pasa por `assemble_plan_node`. Informe: `docs/auditoria_arquitectura_verificacion_2026_09_11.md`.
# [P1-PLAN-FASE-A · 2026-09-11] Fase A del plan de pendientes: el reparto de franjas y el estimador de sodio
...
# [P1-PLAN-LOTE-41 · 2026-09-14] La tormenta de reintentos del catálogo con la base caída: `build_blueprint` hacía
...
# [P1-PLAN-LOTE-169 · 2026-09-23] (frontend por OTA; aquí el marcador y el contrato) encender «Generación de planes» te
...
# [P1-PLAN-LOTE-206 · 2026-09-24] (solo backend) 15 rellenos le dijeron al modelo «el usuario NO consumió… simplifica
# esa franja al máximo» con consumed=0 (nadie registra). «No consumió» sólo con registro representativo, y el bloque
# previo se elige por FECHAS (el shift renumera los días: «window=days 2-6» era UN día de cinco).
...
# [P1-PLAN-LOTE-223 · 2026-09-24] (backend + frontend) a la 1:18 p. m. solo había llegado el aviso del desayuno: la hora
...
# [P1-PLAN-LOTE-224 · 2026-09-24] (backend + frontend) el escáner de comida: la cantidad de un ingrediente no se dejaba
...
# [P1-PLAN-LOTE-225 · 2026-09-25] (backend + frontend) el sistema de idiomas al 100 %: los alimentos en los 5 idiomas
...
async def lifespan(app: FastAPI):
    ...
    if _is_scheduler_leader and HAS_SCHEDULER and scheduler:
        ...
        # [P1-PLAN-LOTE-223] cada 15 min (era a y media): el mensaje del chat sale en el último tick ANTES de que suene
