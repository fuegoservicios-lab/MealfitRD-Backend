...
def test_fototeca_directa_en_la_app_nativa():
    ...
    # [P1-PLAN-LOTE-223] el escáner de comida elige VARIAS fotos (un plato por foto); la fototeca sigue siendo directa
    if (_FRONT / "src" / "__tests__" / "lote223.test.jsx").exists():
        assert "import { chooseNativeGalleryImages, isNativePickerCancellation } from '../../utils/nativeChatImagePicker';" in sm
    else:   # el `main` del frontend que clona el CI del backend, hasta que llegue el lote 223
