# -*- coding: utf-8 -*-
"""[P1-PLAN-LOTE-216 · 2026-09-24] Los recordatorios de comida, a la hora que elige la persona.
...
def test_el_knob_del_historial_esta_apagado_y_documentado():
    ...
    for trozo in ("P1-PLAN-LOTE-216", "MEALFIT_PROACTIVE_NUDGE_FROM_HISTORY", "avisos_por_comida"):
...
def test_el_marcador_va_con_su_lote():
    ...
    assert m and int(m.group(1)) >= 216


def test_configuracion_manda_la_forma_que_el_servidor_valida():
    """El panel del frontend (lote 216) guarda `{meal: {activo, hora: "HH:MM" | null}}` de las CUATRO comidas: la forma
