...
# [P2-HIST-AUDIT-4 · 2026-05-09] Path corregido — post-migración
# P3-CANDIDATE-B los tests viven en `backend/tests/` y este path
# apuntaba a `tests/graph_orchestrator.py` (inexistente). El archivo
# real vive en `backend/graph_orchestrator.py`.
_GO_PATH = Path(__file__).parent.parent / "graph_orchestrator.py"
...
def test_history_capped_at_20(assemble_block):
    """La history debe estar acotada (default 20 entries) para evitar
    bloat ilimitado en `meal_plans.plan_data` (planes con muchos retries
    o múltiples chunks que comparten plan_data).

    [P2-HIST-AUDIT-4 · 2026-05-09] El cap se aplica vía helper SSOT
    `_apply_coherence_history_cap` con knob
    `MEALFIT_COHERENCE_BLOCK_HISTORY_CAP` (default 20). Aceptamos
    AMBOS patrones — el legacy hardcoded `> 20` (por si la migración
    se revierte parcialmente) Y la llamada al helper. Test específico
    de drift en `test_p2_hist_audit_4_coherence_history_cap_knob.py`.
    """
    legacy_hardcoded = (
        re.search(r"len\([^)]+\)\s*>\s*20", assemble_block)
        or "[-20:]" in assemble_block
    )
    helper_invoked = "_apply_coherence_history_cap" in assemble_block
    assert legacy_hardcoded or helper_invoked, (
        "Cap de history no detectado: ni patrón legacy `> 20` / `[-20:]` "
        "ni llamada al helper SSOT `_apply_coherence_history_cap`. Sin "
        "cap, planes con muchos retries pueden inflar plan_data "
        "ilimitadamente."
    )
