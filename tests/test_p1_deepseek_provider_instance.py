"""[P1-DEEPSEEK-PROVIDER-INSTANCE · 2026-07-28] Fix del guard `_is_deepseek_provider`
en `ChatDeepSeek.with_structured_output` (llm_provider.py).

Bug (medido en vivo 2026-07-28): `ChatDeepSeek` es una subclase de `ChatOpenAI`
usada cada vez más contra back-ends OpenAI-compatibles DISTINTOS de DeepSeek
...
`_is_deepseek_provider(base_url)` correctamente — pasando el `base_url` que
ESA instancia recibió. Pero `with_structured_output` (línea ~328) llamaba
`_is_deepseek_provider()` SIN argumento: sin argumento el guard cae al knob
global `MEALFIT_DEEPSEEK_BASE_URL`, que SIEMPRE contiene "deepseek", así que
retornaba `True` para TODA instancia — incluida una `ChatDeepSeek` apuntada a
`https://api.openai.com/v1` — inyectando
`extra_body={"thinking": {"type": "disabled"}}` (parámetro DeepSeek-only) →
...
DeepSeek-only de los ~15 callsites productivos.
...
def _bound_extra_body(runnable):
    ...
    `extra_body` mergeado — ver `ChatDeepSeek.with_structured_output:331`).
...
def test_openai_pointed_instance_does_not_inject_thinking():
    """Una ChatDeepSeek construida contra OpenAI NO debe recibir el
    `extra_body.thinking` DeepSeek-only — de lo contrario OpenAI responde
    400 'Unknown parameter: thinking' (caso real: meal-photo scanner)."""
    instance = lp.ChatDeepSeek(
    ...
    assert extra_body is None or "thinking" not in (extra_body or {}), (
        ...
        f"'thinking' (parámetro DeepSeek-only) — extra_body={extra_body!r}"
    )
...
# ---------------------------------------------------------------------------
# B. Instancia DeepSeek real: SIGUE inyectando `thinking` (anti-regresión).
# ---------------------------------------------------------------------------


def test_deepseek_pointed_instance_still_injects_thinking():
    """Anti-regresión de los ~15 callsites productivos: una ChatDeepSeek
    apuntada al DeepSeek real (base_url default) sigue recibiendo el
    `extra_body.thinking` del que depende el pipeline de generación."""
    instance = lp.ChatDeepSeek(model="deepseek-v4-flash", api_key="sk-test-fake")
    assert "deepseek" in (instance.openai_api_base or "").lower()  # sanity del fixture

    runnable = instance.with_structured_output(_TrivialSchema)
    extra_body = _bound_extra_body(runnable)
    assert extra_body == {"thinking": {"type": "disabled"}}, (
        f"instancia DeepSeek real debe seguir inyectando 'thinking' — "
...
def test_missing_openai_api_base_falls_back_to_global_knob_deepseek_default(monkeypatch):
    """Si `openai_api_base` desapareciera de la instancia (rename futuro en
    langchain-openai), el guard debe caer al knob global — mismo
    comportamiento que existía ANTES de este fix (fail-cheap: nunca debe
    dejar de inyectar 'thinking' para el path DeepSeek-only de producción
    solo porque un rename de atributo tumbó la lectura del instance)."""
    monkeypatch.delenv("MEALFIT_DEEPSEEK_BASE_URL", raising=False)  # default contiene "deepseek"
    ...
    runnable = instance.with_structured_output(_TrivialSchema)
    extra_body = _bound_extra_body(runnable)
    # El knob global (default) SÍ contiene "deepseek" → fallback inyecta,
    # exactamente el comportamiento pre-fix (para NO regresar el path
    # DeepSeek-only si el atributo alguna vez desaparece).
    assert extra_body == {"thinking": {"type": "disabled"}}, (
        f"con el atributo ausente, el guard debe caer al knob global "
        f"(default DeepSeek) — extra_body={extra_body!r}"
    )


def test_missing_openai_api_base_falls_back_to_global_knob_non_deepseek(monkeypatch):
    """Mismo fallback, dirección opuesta: si el knob global apunta a un host
    SIN 'deepseek', el fallback tampoco inyecta — confirma que el fallback
    replica fielmente `_is_deepseek_provider()` sin argumento (el
    comportamiento de hoy), no un hardcode a True."""
    monkeypatch.setenv("MEALFIT_DEEPSEEK_BASE_URL", "https://proxy.example.com")
    ...
    assert extra_body is None or "thinking" not in (extra_body or {}), (
        ...
        f"no-DeepSeek, el fallback NO debe inyectar 'thinking' — "
...
def test_marker_bumped():
    ...
    if "P1-DEEPSEEK-PROVIDER-INSTANCE" in m.group(1):
        return
    fecha = re.search(r"(\d{4}-\d{2}-\d{2})", m.group(1))
    assert fecha and fecha.group(1) >= "2026-07-28", (
        f"marker {m.group(1)!r} anterior a P1-DEEPSEEK-PROVIDER-INSTANCE"
    )
