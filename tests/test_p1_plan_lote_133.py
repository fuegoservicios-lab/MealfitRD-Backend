...
def test_la_hora_del_aviso_es_la_habitual_MENOS_la_antelacion(monkeypatch):
    ...
    [P1-PLAN-LOTE-216] La hora habitual ya no es el camino por defecto (la eligen en Configuración; si no, la normal):
...
def test_el_cron_usa_esa_misma_cuenta_y_no_otra_copia():
    ...
    # [P1-PLAN-LOTE-216] el cron y el teléfono piden la hora a la MISMA función, con la configuración de la persona
...
def test_el_cron_de_avisos_tiene_nombre():
    app = _src("app.py")
    # [P1-PLAN-LOTE-216] cada `MINUTOS_ENTRE_TICKS` (15) en vez de a y media: el chat sale antes que el teléfono
