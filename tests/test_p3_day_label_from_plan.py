...
def _ancla_day_name() -> str:
    """[P1-PLAN-LOTE-224 · 2026-09-24] El backend escribe `day_name` en español («Lunes»): el tab lo sigue leyendo, pero
    lo pinta con `t()` —las mismas claves del cálculo de respaldo—. Hasta que el lote 224 llegue al `main` del
    frontend (el que clona el CI del backend), vale la forma anterior."""
    if (_BACKEND_ROOT.parent / "frontend" / "src" / "__tests__" / "lote224.test.jsx").exists():
