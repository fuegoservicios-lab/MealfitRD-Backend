...
# [2026-07-28] Este test estaba DUPLICADO 22 veces en el archivo (basura de un
# loop de edición previo, ya commiteada — pytest solo ejecuta la última def). Colapsado a UNA.
@patch("cron_tasks.get_user_inventory_net")
@patch("cron_tasks._dispatch_push_notification")
@patch("cron_tasks.execute_sql_write")
# [reapuntado 2026-07-28] El pause por snapshot-stale ahora LEE pipeline_snapshot con
# execute_sql_query ANTES del UPDATE (guard anti-clobber del re-claim) — sin mockearla,
# el test pegaba a la DB REAL con task_id "task_789" (uuid inválido) y moría en psycopg.
@patch("cron_tasks.execute_sql_query", return_value={"pipeline_snapshot": {}})
def test_force_generate_stale_snapshot_triggers_push_and_review(mock_sql_query, mock_sql_write, mock_push, mock_get_inv):
    import cron_tasks
    from datetime import datetime, timedelta, timezone
    ...
    # [reapuntado 2026-07-28 · P0-1 FIX] El force-generate murió: snapshot MUY viejo +
    # live-fetch caído ya NO genera en flexible — PAUSA el chunk ("en lugar de generar a
    # ciegas") con reason=stale_snapshot_live_unreachable, nudge throttled al usuario, y
    # el recovery cron escala a flex+advisory SOLO si el TTL vence sin acción. El contrato
    # de ESTE test pasa a ser la pausa + el review flag.
    assert result.get("_pantry_paused") is True
    assert result.get("_requires_pantry_review") is True
    assert result.get("_fresh_pantry_source") == "stale_snapshot"
    assert not result.get("_pantry_flexible_mode"), (
        "generar en flexible con snapshot >8h y live caído rompe la promesa de nevera"
    )
