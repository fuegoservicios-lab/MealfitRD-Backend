...
def test_el_campo_embebido_sigue_ganando():
    ...
    # [reapuntado 2026-09-24, P1-PLAN-LOTE-225] El gloss pasa a ir en el IDIOMA DEL USUARIO:
    ...
    # Hasta que el lote 225 llegue al `main` del frontend (el que clona el CI del backend),
    # vale la expresión anterior.
    if not (_FRONT / "src" / "__tests__" / "lote225.test.jsx").exists():
    ...
    assert ": (_propio || _embebidoEn || _catalogEn.trim());" in src, (
        ...
        f"embebido [{_MARKER} · P1-PLAN-LOTE-225]"
    )
