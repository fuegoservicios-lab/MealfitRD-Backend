...
def test_chunk_user_locks_insert_accepts_uuid_task_id():
    ...
    # user_profiles o la INSERT lanza ForeignKeyViolation en vez del type-mismatch
    # que este smoke pretende detectar. [P1-E2E-FIXTURE-NEON · 2026-07-10] Neon NO
    # tiene schema `auth`: user_profiles es la raíz (cero FKs) — el INSERT a
    # auth.users mataba este test en setup desde la migración. La propiedad
    # verificada NO cambia: el INSERT del worker con un uuid task_id no debe
    # romper por tipo de `locked_by_chunk_id`.
    execute_sql_write(
    ...
    try:
        ...
        # Teardown FK-safe: locks → user_profiles. (El DELETE de user_profiles
        # cascadea a chunk_user_locks, pero lo hacemos explícito.)
        execute_sql_write(
            "DELETE FROM chunk_user_locks WHERE user_id = %s", (test_user_id,)
        )
        execute_sql_write(
