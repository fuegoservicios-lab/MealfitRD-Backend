...
glm-5.3 estuvo abierto (172 menciones) y el chain de retry del day-gen era
`[glm-5.3]` A SECAS → "Circuit Breaker OPEN para todo el chain" → todos los
...
def test_retry_chain_has_two_model_net(monkeypatch):
    ...
    assert chain == ["glm-5.3-flash", "glm-5.3"], \
    ...
    assert g._day_model_chain(_NON, 3) == ["glm-5.3-flash", "glm-5.3"]
...
def test_bariatric_has_pro_net(monkeypatch):
    ...
    assert g._day_model_chain(_BAR, 2) == ["glm-5.3-flash", "glm-5.3"], \
