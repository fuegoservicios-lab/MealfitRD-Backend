...
Las pruebas funcionales de la pantalla viven en `frontend/src/__tests__/lote224.test.jsx`; aquí, además, las anclas
que el frontend tiene que conservar (el test las salta mientras el frontend de al lado no traiga el lote 224).
...
# El CI del backend clona el `main` del frontend (su checkout no fija `ref:`). Mientras el lote 224 no llegue allí, ese
...
def test_el_marcador_esta_al_dia():
    ...
    assert "P1-PLAN-LOTE-224" in (_BACKEND / "docs" / "diario_registrar_comida.md").read_text(encoding="utf-8")
