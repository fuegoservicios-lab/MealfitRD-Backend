"""[P1-PLAN-LOTE-224 · 2026-09-24] Textos cortos que el motor escribe en español, traducidos PARA LEER.
...
# [P1-PLAN-LOTE-224] Segundo tipo: frases cortas que la app enseña al usuario y no son nombres (lo que el coach
...
def _validar(res, limpios: list, node: str, max_chars: int = _MAX_TEXTO) -> Optional[list]:
    ...
    if len(traducidos) != len(limpios):
        logger.warning(f"[P1-PLAN-LOTE-224] traducción descartada ({node}): {len(traducidos)} textos para {len(limpios)}")
        return None
    out = []
...
async def traducir_para_mostrar(textos, locale, *, user_id: Optional[str] = None,
    ...
    try:
        ...
        logger.warning(f"[P1-PLAN-LOTE-224] traducción para mostrar no disponible: {e}")
        return None
    try:
        res = await asyncio.wait_for(llm.ainvoke(mensajes), timeout=timeout_s + 1)
    except Exception as e:
        logger.warning(f"[P1-PLAN-LOTE-224] traducción para mostrar falló ({node}, {locale}): {type(e).__name__}: {e}")
        return None
    finally:
        reset()
