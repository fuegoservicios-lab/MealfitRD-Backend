...
- **Provider LLM: DeepSeek V4 + router por tier** [P0-DEEPSEEK-MIGRATION · 2026-06-12]: Gemini eliminado por completo (dep, embeddings, vision, safety_settings). SSOT [`backend/llm_provider.py`](backend/llm_provider.py): tier `gratis`/guests → `deepseek-v4-flash`; `basic`/`plus`/`ultra` → `deepseek-v4-pro` (**fail-cheap**: cualquier duda/fallo de lookup → flash; el reviewer médico risk-tier va a PRO para TODOS los tiers). Key env `DEEPSEEK_API_KEY` (base OpenAI-compatible `https://api.deepseek.com`). Tabla canónica surfaces/knobs/pricing: [`backend/docs/llm_tier_routing.md`](backend/docs/llm_tier_routing.md). Test ancla: [`test_p0_deepseek_migration.py`](backend/tests/test_p0_deepseek_migration.py).
- **Embeddings: Cohere Embed v4** [P1-COHERE-EMBED-V4 · 2026-06-12]: stack de DOS providers (DeepSeek LLM + Cohere embeddings). SSOT [`backend/embeddings_provider.py`](backend/embeddings_provider.py): `embed-v4.0` @1536 (= columnas pgvector tras migración [`p1_cohere_embed_v4_vector_dims_2026_06_12.sql`](supabase/migrations/p1_cohere_embed_v4_vector_dims_2026_06_12.sql), aplicada 2026-06-12 — vectores Gemini legacy anulados). **Asimetría `input_type`** (palanca de precisión RAG): queries→`search_query`; persistido en pgvector (`user_facts`, `visual_diary`)→`purpose="document"`→`search_document`. **Cache keys versionadas** por `get_embeddings_model_id()` (`embed-v4.0@1536`)+purpose — sin esto Redis (TTL ~100 años) serviría vectores del espacio anterior tras un switch. Gating por presencia de `COHERE_API_KEY` (sin key ⇒ degradación limpia keyword/recency; activación = key + restart). Vision sigue `disabled` (requiere VLM; Embed v4 sí soporta embeddings de imagen para búsqueda visual futura). Test ancla: [`test_p1_cohere_embed_v4.py`](backend/tests/test_p1_cohere_embed_v4.py).
