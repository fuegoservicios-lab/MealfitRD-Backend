...
[P1-COUNTRY-SYSTEM-F1 · 2026-08-16] Con el knob encendido, el motor deja de forzar lo criollo en beta (prompts/gates/jueces/tz/presupuesto/precios). Doc: backend/docs/country_system_f1.md. Spec: docs/superpowers/specs/2026-08-16-sistema-paises-design.md. Test test_p1_country_system_f1.py.

[P1-COUNTRY-SYSTEM-F2 · 2026-08-18] Catálogo completo (347 filas, 6 países) + retry-gate advisory desde el intento 1 para país beta + MUTATOR-PURITY cerrado (swap/recalc). QA final con LLM vivo verde (5 beta + gemelo DO) y harness 0/0 en los 5. Doc: backend/docs/country_system_f1.md §F2 (incluye el RUNBOOK DEL FLIP). Test test_p1_country_system_f2.py.

[P0-ALLERGEN-VOCAB-I18N + ola de países · 2026-08-21] Auditoría con el flip YA vivo: 107 gaps. **El catálogo de F2 era INERTE para la generación** (el bloque «USA EXCLUSIVAMENTE ESTOS ALIMENTOS» filtra por precio RD y las 141 filas beta nacen sin precio) y **la costura (a) estaba mal diagnosticada**: no es léxico DO-tuned sino un espejo sin dos ramas, más viejo que el sistema de países. El backstop de alergias era ciego a «cacahuete»/sésamo/inglés: `_ALLERGEN_SYNONYMS` es «cómo se LLAMA», `_ALLERGEN_DECLARATION_ALIASES` «cómo lo DECLARA». Plan, gaps abiertos y los tests de cada cierre: [`docs/superpowers/plans/2026-08-20-paises-produccion-gaps.md`](docs/superpowers/plans/2026-08-20-paises-produccion-gaps.md).

[P1-COUNTRY-SYSTEM-F0 · 2026-08-16] Fase 0 del sistema de países EN OSCURO: knob maestro `MEALFIT_COUNTRY_SYSTEM` (False) + bandera frontend `VITE_COUNTRY_SYSTEM` ausente ⇒ byte-idéntico a ayer, el dato se escribe pero nada lo lee para decidir conducta. **`constants.canonicalize_country` es el ÚNICO SSOT** — no escribas otra tabla (la lección de P1-DIET-CANON-SSOT). El lector preexistente (cold-start en `get_similar_user_patterns`) quedó tras `MEALFIT_COUNTRY_COLDSTART_SEGMENT` (False). Spec: docs/superpowers/specs/2026-08-16-sistema-paises-design.md. Test test_p1_country_system_f0.py.
