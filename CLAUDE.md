...
- **Provider LLM: DeepSeek V4, flash primario** [P0-DEEPSEEK-MIGRATION · 2026-06-12 · P1-FLASH-PRIMARY · 2026-07-31]: Gemini eliminado. SSOT [`backend/llm_provider.py`](backend/llm_provider.py): **TODOS los tiers → `deepseek-v4-flash`** (owner: flash hoy es MEJOR que pro; incluye el reviewer risk-tier). Pro = SOLO red post-fallo (breaker independiente — NO colapsarla a flash). Rollback `MEALFIT_MODEL_PAID_TIER`. Reviewer clínico por tier [P1-REVIEWER-TIER-MODELS · 2026-07-31]: free→`gpt-5.6-luna`, pagados→`gpt-5.6-terra` (recorte OpenAI -80%/-20%; fail-safe sin key → flash). Detalle/knobs/pricing: [`backend/docs/llm_tier_routing.md`](backend/docs/llm_tier_routing.md). Tests: [`test_p1_flash_primary.py`](backend/tests/test_p1_flash_primary.py), [`test_p1_reviewer_tier_models.py`](backend/tests/test_p1_reviewer_tier_models.py).
