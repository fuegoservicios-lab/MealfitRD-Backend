...
| `PATCH /api/profile` ([routers/user_data.py](backend/routers/user_data.py)) | **[P3-I18N-PROFILE-EXENCION-SIN-FILA · 2026-08-22]** El merge del perfil, y el ÚNICO exento que puede acabar gastando en el LLM: cambiar `locale` dispara el enriquecimiento de `_display`. Va exento igual — al cap, un 402 aquí dejaría al usuario sin poder corregir su perfil ni volver a su idioma, y `get_monthly_api_usage` cuenta toda fila de `api_usage` sin filtrar endpoint. Ese gasto NO va a `api_usage`: vive en `llm_usage_events` (`node="plan_display_i18n"`), el libro de COSTO, no el de créditos. `Depends(_PROFILE_PATCH_LIMITER)`. |
