...
[P1-ARQ25-F5-PLAN-JOBS · 2026-09-04] Fase 5 (rebanada 1): worker del outbox `plan_jobs` (claim `FOR UPDATE SKIP LOCKED`, `attempts` como fencing, backoff, dead letter, reclaim por heartbeat, `stale` + re-encolado si `meal_plans.revision` cambió) y consumidor `display_i18n` (el disparador encola en vez de abrir un hilo; guests siguen legacy). Los `job_type` sin consumidor quedan `pending`. Knob maestro `MEALFIT_PLAN_JOBS_ENABLED` (False). Doc `backend/docs/plan_jobs_f5.md`; test `test_p1_arq25_f5_plan_jobs.py`.
