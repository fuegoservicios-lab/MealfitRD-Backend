...
| `/api/diary/upload` ([routers/diary.py](backend/routers/diary.py)) | **[P1-MEAL-SCAN-GEMMA · 2026-07-12 → P1-VISION-LUNA · 2026-07-28]** "Escanear comida" → gemma LOCAL (costo cero) o provider CLOUD pago (Luna). `Depends(_VISION_UPLOAD_LIMITER)` (10/60s). El gasto de un scan cloud YA NO va a `api_usage` (`log_api_usage` salió del call site) — ahora vive en `llm_usage_events` (libro de COSTO) vía `log_llm_usage_event(node="vision_scan")`, así un scan nunca quema crédito de plan. Tests [`test_p1_meal_scan_gemma.py`](backend/tests/test_p1_meal_scan_gemma.py), [`test_p1_vision_luna.py`](backend/tests/test_p1_vision_luna.py). |
