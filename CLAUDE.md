...
## Lifecycle único de generación + PlanPolicy (roadmap 2.5, Fases 1-3)
...
- **CI con veredicto** [P0-CI-VERDICT · 2026-09-04]: lo que un checkout no puede evaluar (hermano frontend, artefactos no versionados, catálogo vivo) se SALTA con razón; `.env.example` es el espejo versionado de los knobs afinados. Audit P0–P3: [`docs/gaps-audit-2026-09.md`](backend/docs/gaps-audit-2026-09.md).
...
- **Provider LLM: Z.ai GLM-5.3 + gpt-5.6 por tier** [P0-GLM-MIGRATION · 2026-09-02 · P1-FLASH-PRIMARY · 2026-07-31]: flash=`glm-5.3-flash`, pro=`glm-5.3`, effort `MEALFIT_GLM_REASONING_EFFORT`=low. SSOT [`backend/llm_provider.py`](backend/llm_provider.py): day-gen por tier [P1-DAYGEN-TIER-MODEL]: plus/ultra→Luna medium, resto→Luna low; swap y `/regenerate-day` [P1-SWAP-LUNA · 2026-08-05]: Luna fijo, effort por superficie (individual `medium` ~16,5s / día `low` ~8,2s = igual que flash — el día es bucle EN SERIE de 4-5); demás nodos flash. Red post-fallo CROSS-PROVIDER: `gpt-5.6-luna` [P1-NET-LUNA] (sin OPENAI_API_KEY → pro; NO colapsarla a flash). Rollback `MEALFIT_MODEL_PAID_TIER`/`MEALFIT_PRO_MODEL`. Reviewer clínico por tier [P1-REVIEWER-TIER-MODELS + P1-REVIEWER-SOL-HARD · 2026-07-31]: free→Luna, pagados→Terra, plus/ultra difícil (bariátrico/≥2 reglas SSOT)→Sol (fail-safe sin key → flash). Detalle: [`backend/docs/llm_tier_routing.md`](backend/docs/llm_tier_routing.md). Tests: [`test_p1_flash_primary.py`](backend/tests/test_p1_flash_primary.py), [`test_p1_reviewer_tier_models.py`](backend/tests/test_p1_reviewer_tier_models.py).
...
- **SSOT de migrations** [P3-MIGRATIONS-SSOT · 2026-05-20]: TODA migration vive en DOS directorios mantenidos sincronizados — `migrations/` (workspace-root, cross-repo) Y `backend/migrations/` (backend repo). Al añadir una nueva migration, copiarla a AMBOS dirs en el mismo commit/push de cada repo. Razón: el workspace-root usa el `.gitignore` que excluye `backend/`+`frontend/` (son repos hermanos con remotes propios), así que necesitas archivos físicos en cada dir para que cada `git push` lleve el cambio. Drift histórico (audit 2026-05-20): 4 files root-only + 1 file backend-only fueron sincronizados; estado actual = 37 archivos idénticos en ambos. Verificación rápida: `diff <(ls migrations) <(ls backend/migrations)` debe retornar vacío. **[P2-I18N-MIGRACIONES-SIN-LIBRO · 2026-08-23] Libro `public.schema_migrations`**: `scripts/apply_migration.py --status` dice qué falta (exit 4 = pendientes); `--apply` anota; `--record` anota sin ejecutar (di cómo lo verificaste). Medido al crearlo: 1 de 110 sin aplicar en prod. SOP: [`backend/docs/migrations_ledger.md`](backend/docs/migrations_ledger.md).
- **Convención de imports DB** [P3-DB-IMPORTS-FACADE · 2026-05-20]: nuevos call sites usan la **fachada** `from db import <funcion>` ([`backend/db.py`](backend/db.py)), no los sub-módulos (`from db_plans import ...`): `db.py` re-exporta los sub-módulos y es el contrato público; el sub-módulo directo acopla la organización interna al call site y elude los `__all__` (sentinel: `test_p3_new_star_imports_audit.py`). Sin grep+replace masivo; política "boy scout" al editar un fichero con `from db_<sub> import`. Los sub-módulos siguen siendo SSOT; la fachada es solo la API pública.
...
- **`TODO`/`TODOS` en comentarios — solo marker de deuda**: [P3-TODOS-NARRATIVE · 2026-05-13] mayúsculas (`TODO`/`FIXME`/`XXX`/`HACK`) solo para trabajo pendiente real; el sustantivo "todo/todos" va en minúscula (audit 2026-05-12: 243 matches, casi todos el sustantivo). Convención editorial, sin enforcement.
...
- **Dev local — auto-reload del backend**: `UVICORN_RELOAD=1` en `.env` evita el modo de fallo «fix en HEAD, binario viejo» (sin reload, editar `constants.py`/`routers/*.py` exige restart manual). Default `0` en prod (P2-UVICORN-RELOAD-ENV). Verificación: `curl /health/version` → `last_known_pfix` vs HEAD. SOP: [`runbook_dev_local_setup_2026_05_23.md`](~/.claude/projects/.../memory/runbook_dev_local_setup_2026_05_23.md).
- **SQL forense antes de tocar código**: si el bug depende de datos persistidos (`plan_data`, `user_inventory`, `master_ingredients`), ejecuta el SELECT **antes** de teorizar. DB = **Neon**: `load_dotenv()` + `psycopg.connect(os.environ['NEON_DATABASE_URL'])` desde un script. ⚠️ Fuera de FastAPI hay que **abrir el pool** (`db_core.connection_pool.open()`) o `master_ingredients` sale vacío. Templates: [`runbook_sql_forensic_sop_2026_05_23.md`](~/.claude/projects/.../memory/runbook_sql_forensic_sop_2026_05_23.md).
- **Soft-fail pattern (HTTP 200 + body flag)** [P3-SWAP-SOFT-FAIL-200 · 2026-05-23]: si el «fallo» es business-as-usual (LLM no convergió), 200 con `operation_failed:true` + `error_code` canónico + `error_message` (sin rojo en DevTools; logger.warning + knob). NO para validation/auth/not-found (siguen 4xx). Criterios y endpoints: [`runbook_soft_fail_pattern_2026_05_23.md`](~/.claude/projects/.../memory/runbook_soft_fail_pattern_2026_05_23.md).
...
| `/shift-plan` ([routers/plans.py](backend/routers/plans.py)) | **[P3-SHIFT-PLAN-QUOTA-EXEMPT · 2026-06-15]** Avance de la ventana rolling de un plan YA generado (mantenimiento): un 402 aquí congelaba un plan ya pagado. `Depends(_SHIFT_LIMITER)` (20/60s), NO cuenta contra el cap; anti-hammering (P2-LIVE-7) vía RateLimiter+idempotencia. Test [`test_p3_shift_plan_quota_exempt.py`](backend/tests/test_p3_shift_plan_quota_exempt.py). |
| `/restock` ([routers/plans.py](backend/routers/plans.py)) | **[P1-NEVERA-QUOTA-EXEMPT · 2026-06-24]** «Ya compré la lista» → INSERT/UPDATE `user_inventory`. Cero costo LLM; antes al cap congelaba la Nevera Y quemaba crédito de planes. `Depends(_RESTOCK_LIMITER)` (20/60s), NO cuenta contra el cap. Test [`test_p1_nevera_quota_exempt.py`](backend/tests/test_p1_nevera_quota_exempt.py). |
| `/inventory/consume` ([routers/plans.py](backend/routers/plans.py)) | **[P1-NEVERA-QUOTA-EXEMPT · 2026-06-24]** Vaciar consumidos (`quantity=0`), sub-paso de renovar plan (`useRegeneratePlan.js`). Cero costo LLM; antes un 402 abortaba la renovación entera. `Depends(_CONSUME_LIMITER)` (20/60s), NO cuenta contra el cap. Test [`test_p1_nevera_quota_exempt.py`](backend/tests/test_p1_nevera_quota_exempt.py). |
| `/api/diary/upload` ([routers/diary.py](backend/routers/diary.py)) | **[P1-MEAL-SCAN-GEMMA · 2026-07-12 → P1-VISION-LUNA · 2026-07-28 → P1-VISION-NO-LOCAL · 2026-07-28 → P1-VISION-GEMINI-FLASH · 2026-09-04]** «Escanear comida» → provider CLOUD (Gemini 3.8 Flash). `Depends(_VISION_UPLOAD_LIMITER)` (10/60s). El gasto va a `llm_usage_events` (`log_llm_usage_event(node="vision_scan")`), NUNCA a `api_usage`. Tests [`test_p1_vision_luna.py`](backend/tests/test_p1_vision_luna.py), [`test_p1_vision_no_local.py`](backend/tests/test_p1_vision_no_local.py), [`test_p1_vision_gemini_flash.py`](backend/tests/test_p1_vision_gemini_flash.py). |
| `POST /api/diary/consumed-from-plan` ([routers/diary.py](backend/routers/diary.py)) | **[P1-EAT-PLAN-MEAL · 2026-08-07]** «Me lo comí» → registra el plato del plan en el diario y descuenta sus ingredientes de la Nevera. Cero costo LLM (SELECT + INSERT + restas). `Depends(_PLAN_MEAL_LIMITER)` (20/60s). Test [`test_p1_eat_plan_meal.py`](backend/tests/test_p1_eat_plan_meal.py). |
| `DELETE /api/diary/consumed/{meal_id}` ([routers/diary.py](backend/routers/diary.py)) | **[P1-DIARY-EDITABLE · 2026-07-28]** «Deshacer registro» → `DELETE consumed_meals` filtrado por `user_id`. Cero costo LLM; al cap el usuario debe poder CORREGIR su propio error. `Depends(_DELETE_CONSUMED_LIMITER)` (20/60s), NO cuenta contra el cap. Test [`test_p1_diary_editable.py`](backend/tests/test_p1_diary_editable.py). |
| `POST /api/diary/consumed-from-plan/preview`, `POST /api/diary/plan-meal-deviation` ([routers/diary.py](backend/routers/diary.py)) | **[P1-EAT-PLAN-MEAL-TRUTH · 2026-09-04]** Vista previa de cobertura de la Nevera y desvío declarado («comí otra cosa» / «todavía no» → `pipeline_metrics` node `plan_meal_deviation`). Cero LLM. `Depends(_PLAN_MEAL_PREVIEW_LIMITER/_PLAN_MEAL_DEVIATION_LIMITER)` (30/20 por 60s). Test [`test_p1_eat_plan_meal_truth.py`](backend/tests/test_p1_eat_plan_meal_truth.py). |
| `POST /api/diary/consumed/estimate-macros` ([routers/diary.py](backend/routers/diary.py)) | **[P1-DIARY-FREETEXT-ESTIMATE · 2026-09-04]** Texto libre → macros estimadas (flash); exenta igual: el gasto va a `llm_usage_events`, NUNCA a `api_usage`. `Depends(_ESTIMATE_MACROS_LIMITER)` (10/60s). Test [`test_p1_diary_freetext_estimate.py`](backend/tests/test_p1_diary_freetext_estimate.py). |
| `POST /api/diary/consumed/manual`, `GET /api/diary/foods/frequent`, `POST /api/diary/consumed/repeat` ([routers/diary.py](backend/routers/diary.py)) | **[P1-MANUAL-FOOD-LOG · 2026-08-11]** El componedor del diario (catálogo, frecuentes, repetir). Cero costo LLM. `Depends(_MANUAL_MEAL_LIMITER/_FOOD_FREQUENT_LIMITER/_REPEAT_MEAL_LIMITER)` (20-30/60s); `POST /api/diary/consumed` ganó `_CONSUMED_WRITE_LIMITER`. Test [`test_p1_manual_food_log.py`](backend/tests/test_p1_manual_food_log.py). |
| `GET/PUT /api/profile/plan-mode`, `GET /api/nutrition/targets` ([routers/user_data.py](backend/routers/user_data.py)) | **[P1-PLAN-MODE · 2026-08-11]** Interruptor plan↔seguimiento y metas del contador. Cero costo LLM. El PUT es la puerta de REANUDAR (un 402 atraparía al usuario en pausa). `Depends(_PLAN_MODE_LIMITER)` (15/60s) / `Depends(_TARGETS_LIMITER)` (30/60s). Test [`test_p1_plan_mode.py`](backend/tests/test_p1_plan_mode.py). |
| `PATCH /api/profile` ([routers/user_data.py](backend/routers/user_data.py)) | **[P3-I18N-PROFILE-EXENCION-SIN-FILA · 2026-08-22]** Merge del perfil; el ÚNICO exento que puede gastar LLM (cambiar `locale` enriquece `_display`) y ese gasto vive en `llm_usage_events` (`node="plan_display_i18n"`), no en `api_usage`. Al cap el usuario debe poder corregir su perfil e idioma. `Depends(_PROFILE_PATCH_LIMITER)`. |
| `GET /api/chat/quota` ([routers/chat.py](backend/routers/chat.py)) | **[P1-COACH-QUOTA-METER · 2026-09-02]** Lectura de la cuota MENSUAL del coach para el medidor del chat. Cero LLM. Al tope, el usuario necesita VER cuánto le queda y cuándo se renueva, no otro 402. `Depends(_COACH_QUOTA_LIMITER)` (30/60s). Test [`test_p1_coach_quota_meter.py`](backend/tests/test_p1_coach_quota_meter.py). |
| `/generation-eta` ([routers/plans.py](backend/routers/plans.py)) | **[P2-LOADING-ETA-HONEST · 2026-09-03]** p50/p90 real del bloque 1 (14 d) para la pantalla de carga; el «3-6 min» fijo mentía (mediana 10, p90 15). Cero LLM, cache 10 min, `Depends(_ETA_LIMITER)` (30/60s). Test [`test_p2_loading_eta_honest.py`](backend/tests/test_p2_loading_eta_honest.py). |
| `GET /api/plans/{plan_id}/projections` ([routers/plans.py](backend/routers/plans.py)) | **[P1-ARQ25-F5-SHOPPING-PROJECTION · 2026-09-04]** Estado (`none/pending/ready/failed/stale`) y read model de la proyección de compras por revisión (`plan_jobs`). Cero LLM. `Depends(_PROJECTIONS_LIMITER)` (30/60s). Test `test_p1_arq25_f5_shopping_projection.py`. |

**Por qué no `verify_api_quota`:** el paywall mensual (gratis=10, basic=50, plus=200, ultra=500 — P1-CREDITS-LADDER) devuelve `HTTP 402` al exceder; aplicarlo a lecturas o a operaciones sin costo LLM bloquearía al usuario en lo que ya pagó (`get_monthly_api_usage` cuenta toda fila de `api_usage`: cada llamada quemaría crédito). Anti-spam = `RateLimiter` per-bucket, NO el paywall. Tests [`test_p1_audit_3_history_quota_exemption.py`](backend/tests/test_p1_audit_3_history_quota_exemption.py) (3 rows originales) + [`test_p3_pdf_polish_4.py`](backend/tests/test_p3_pdf_polish_4.py) (2 rows del bundle PDF) anclan ambas decisiones.
...
[P1-IOS-NATIVE-SHELL · 2026-08-21] Wrapper Capacitor (`frontend/ios/`, `com.bioboros.app`). Apple 3.1.1: pago sólo web; en nativo **no existe comercio** (precios, «Mejorar plan», PayPal, landing). **UN gate** `frontend/src/config/platform.js`. `viewPlansLabel={null}` NO bastaba (el `??` lo rellenaba). Spec `2026-08-21-ios-native-shell-design.md` (workspace, no versionada); test [`test_p1_ios_native_shell.py`](backend/tests/test_p1_ios_native_shell.py).
...
[P1-I18N-DASHBOARD · 2026-08-15 · alcance corregido 2026-08-22] La app se lee en 5 idiomas (es-DO base, en-US, pt-BR, fr-FR, it-IT); selector en Configuración → Idioma. **No es solo la interfaz**: desde el 19-ago la capa `_display` traduce el CONTENIDO del plan (platos, descripciones, recetas, insights), y siguen el idioma el coach (chat + push + help bot) y el PDF. La regla no es «lo que escribe el LLM no se toca», sino **«lo que el motor usa como IDENTIFICADOR no se toca»**. NO se traducen los legales (traducir un contrato genera obligaciones) ni **los nombres de alimentos, JAMÁS**: son el SSOT del motor — `pantry_names_match`, guard de coherencia y backstop de alergias resuelven por esos nombres exactos, así que traducir «Pollo» rompe las tres, dos en silencio.
...
### `chat-agent safety_settings relajados` (SUPERSEDED por GLM)

[P3-CHAT-SAFETY-OFF-DECISION · 2026-05-20 · superseded P0-LLM-PROVIDER-MIGRATION 2026-06-12] Aplicaba a los content-filters de Gemini (`DANGEROUS_CONTENT` + `safety_settings`) por false-positives en charlas de déficit/ayuno. GLM no los expone client-side: el bloque salió de [agent.py](backend/agent.py) y la intención la cubre el default del provider. Detalle: memoria `project_p3_chat_safety_off_decision_2026_05_20`.

### `P3-LANDING-DARK-ONLY` (landing oscuro fijo) — SUPERSEDED

[P2-PAPER-NO-INK · 2026-08-01] Las 6 rutas de marketing pasan de oscuro a blanco y negro estricto («papel técnico») vía un 4º `data-theme="paper"`; dashboard intacto. `P3-LANDING-DARK-ONLY` (landing sin configuración de apariencia, siempre oscuro) queda **SUPERSEDED**: el dueño la anuló a sabiendas del precedente en `feedback_marketing_design_minimalist_scientific.md` (dos reverts 2026-07-02 pedían NO monocromatizar los centerpieces). Spec + estado real (nada mergeado aún, benchmark pendiente de decisión): `2026-08-01-landing-papel-tecnico-design.md` (workspace, no versionada).

### `P1-HERO-DEDUP-ACCENT` (el acento del landing) — supersede parcial de `P2-PAPER-NO-INK`

[P1-HERO-DEDUP-ACCENT · 2026-08-09] Nace `--pa-accent: #C1200E` (5,83:1 contra el papel en AMBAS direcciones), la única tinta de color del sistema. **Marca LA CIFRA de un SSOT — condición necesaria, no suficiente**: 2 call sites (cotas de la Fig. 00, numeral de créditos), nunca en un control ni en el CTA (el botón ya es la única tinta sólida de la pantalla: teñirlo REBAJA la jerarquía). Cierra además 7 duplicaciones hero↔header↔franja, incluida la franja de 5 celdas de la que 4 reformulaban el párrafo de encima. El titular perdió su `<br />` fijo: `text-wrap: balance` es load-bearing (sin él vuelve la línea huérfana). Spec: `2026-08-09-hero-landing-llamativo-design.md` (workspace, no versionada). Test [`test_p1_hero_dedup_accent.py`](backend/tests/test_p1_hero_dedup_accent.py).
...
Verificar: `pytest backend/tests/test_p1_new_a_frontend_no_direct_meal_plans_write.py -v` — bloquea `update|delete|upsert` no-whitelisted; permite `insert`.
...
Verificar: `pytest backend/tests/test_p0_agent_1_user_id_override.py backend/tests/test_p2_chat_cleanup.py -v`.
...
Reglas anti-refactor del **landing y el apex** (8: service worker diferido, observabilidad por HOST, preload gateado, SSOT de precios/sitemap, dieta de `lucide`, `@sentry` fuera del entry, precache por marcador de paquete) — movidas a [`backend/docs/landing_apex_antipatterns.md`](backend/docs/landing_apex_antipatterns.md) porque vivían bajo un título sobre el motor de planes. Plan de producción del landing (25 gaps): `2026-08-14-landing-produccion-design.md` (workspace, no versionada).
...
[P0-SHOPPING-CYCLE-DAYS · 2026-08-22] La lista se agregaba desde `plan_data["days"]` — la ventana VIVA, que el shift ENCOGE — y cada recálculo la reescribía más corta: 48 alimentos → **25**, el conjunto exacto del último día superviviente; la nevera nació como su espejo (UNA proteína) y el chunk murió en el gate. **El generador NO tuvo la culpa** (48 vs mediana de flota 46). SSOT `shopping_source_days`, usado por LOS DOS lados: el guard tomaba su referencia del MISMO `days` encogido, así que mutilar la lista MEJORABA su métrica (31→6). Knob `MEALFIT_SHOPPING_SOURCE_INCLUDES_ARCHIVED`. Doc: backend/docs/shopping_list_cycle_days.md. Test test_p0_shopping_cycle_days.py.

[P1-POSTMERGE-WAIVER-SSOT + P1-PANTRY-CONDIMENT-PARITY + P2-SHOPPING-PROTEIN-FLOOR · 2026-08-22] La **CUARTA** guarda de nevera decidía sola y honraba 1 de los 4 waivers: un `initial_plan` ya perdonado por las guardas 1 y 3 moría ahí con `attempts=0` (**al LLM nunca se le dijo nada**). **El blanket que debía cazarlo recorría una tupla de UN elemento.** Ahora delega en `_pantry_gate_waiver_reason` y con waiver anota y entrega; `_p0_4_violations` deja de ser escritura muerta. El prompt autorizaba cebolla/comino/laurel/polvo de hornear y `_ALLOWED_CONDIMENTS` no tenía ninguna: se castigaba al modelo por obedecer (**no la fusiones con `culinary_coherence.CONDIMENT_EXEMPT`**). Y el piso contaba NOMBRES: 25 ítems con UNA proteína pasaban limpios. Doc: backend/docs/pantry_gate_fourth_guard.md.
...
**SOP operador post-merge**: `git push` → merge → redeploy en el VPS Oracle (deploy-mealfit.ps1) → `curl POST /api/system/admin/deploy-lag/check` (auth Bearer CRON_SECRET) hasta `drift=false` (`drift=true` = binario rezagado: deploy no aplicado o rollback). Bumpear `expected_last_known_pfix` solo tras `drift=false`. Test: [`test_p0_prod_1_deploy_force_check.py`](backend/tests/test_p0_prod_1_deploy_force_check.py).
