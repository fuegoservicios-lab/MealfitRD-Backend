...
- **Modelos LLM via knob, no hardcoded**: [P3-PREVIEW-MODEL-KNOB · 2026-05-12] callsites en crons/loops productivos leen model ID desde knob `MEALFIT_<FEATURE>_MODEL` via helper `_<feature>_model_name()`. Razón histórica: modelos preview de Google se depreciaban sin aviso (CB row stale 4.4 días, audit 2026-05-11); sigue vigente con GLM (aliases legacy deprecan 2026-07-24). El override per-feature SIEMPRE gana sobre el router por tier — rollback/A-B sin redeploy.
...
- **Soft-fail pattern (HTTP 200 + body flag)** [P3-SWAP-SOFT-FAIL-200 · 2026-05-23]: cuando el «fallo» es business-as-usual (LLM no convergió), responder 200 con `operation_failed:true` + `error_code` canónico + `error_message` — sin ruido rojo en DevTools y con observabilidad (logger.warning + knob). NO para validation/auth/not-found (siguen 4xx). Criterios, templates y endpoints bajo el pattern: [`runbook_soft_fail_pattern_2026_05_23.md`](~/.claude/projects/.../memory/runbook_soft_fail_pattern_2026_05_23.md).
