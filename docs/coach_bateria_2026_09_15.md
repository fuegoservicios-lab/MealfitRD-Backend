...
**Lote 80 (`P1-PLAN-LOTE-80`, la franja no se pregunta).** B6 («ayer me comí un chimi en la calle») registró como cena de ayer en las corridas A y B y en la C preguntó «¿fue el almuerzo o la cena?». La instrucción de `log_consumed_meal` (en sus dos variantes) dice ahora que la franja tampoco se pregunta: se asume la más probable por el plato y la hora, se registra y se dice; la corrección va por `correct_consumed_meal`. Mini-batería de los 10 casos de registro (B1, B3, B4, B6, B8, H2, J2, K3, K4, K7), US$ 0,03: los 10 registran, y B6 queda como cena de AYER con la oferta del almuerzo de ayer (80 palabras).

**Lote 81 (`P1-PLAN-LOTE-81`, el idioma del mensaje manda).** Batería final con los lotes 77-80 desplegados (US$ 0,10, 775 s): 8/63 con banderas, 5/63 sobre el tope (era 20), 0 FD1/FD2/FD3 — pero I5 (app en es-DO, mensaje en inglés) salió en español: FD4 por varianza (en las otras tres corridas fue en inglés). La regla F del prompt no basta; la directiva nativa por locale (P1-COACH-LANGUAGE-NATIVE) sí se obedece, así que ahora se añade POR TURNO cuando el mensaje llega en otro idioma que el de la app (`detect_message_locale` + `build_message_language_directive`, marcadores sin palabras españolas). Mini-batería I1-I5 + A1 + B1 para confirmarlo.

**Lote 84 (`P1-PLAN-LOTE-84`, el cierre sabe qué hora es).** El dueño registró el desayuno a las 11:23 y el coach cerró con «¿Te anoto el almuerzo cuando lo comas?»: la nota del lote 76 ofrecía LA QUE FALTA por su nombre sin mirar si ya tocaba. Ahora la nota de `log_consumed_meal` separa lo que falta HOY por la hora local: lo que ya pasó su hora (45 min de gracia tras la hora típica, que es la media real del usuario dentro de su franja o la de siempre) se ofrece por su nombre como antes; lo que aún no ha llegado se cierra con UNA frase consciente de la hora y sin pregunta («ya casi es hora de almorzar: cuando almuerces, cuéntame qué comiste y lo anoto»; «ya casi» solo si falta ≤ 2 h). Ayer no cambia. Mini-batería de registro para confirmarlo.
