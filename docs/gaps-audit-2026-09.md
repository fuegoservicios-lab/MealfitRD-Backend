...
| P1-4 | **Drift real de tests (rojos en la máquina del dueño también)**: contextos legacy (5º path `seed_chunk1_queue`), drain endpoint con rate-limit, `pytest` sin importar (2), conteo de vitest 319→360, marker de check-in bumpeado, SELECT con `attempts`, 4 ficheros del LoadingScreen anclando el diseño anterior (P2-LOADING-ONE-STROKE/ETA-HONEST los supersedió), deadline de embeddings con defaults distintos al `.env`, meta-guard de humanize. | 22 tests. | Cada test reanclado al diseño vigente con la supersesión escrita; huella durable de `P2-CHECKIN-NO-FABRICATED-ANSWERS` en `app.py`. 05/09: `main` dejó 4 anclas más en rojo (runs 1547-1561) y se reanclan aquí: huella de `P2-CHAT-ERROR-MINIMAL` (8f96969 bumpeó el marker sin dejarla), gate anidado del botón «Reintentar» (`isErrorBubble` + `msg.retryable`, misma intención), feedback «Detenido» sin emoji, y el espaciador del anclaje (`spacerRef`) como ÚNICA escritura de altura tolerada por el guard del autosize. |
...
| P1-6 | **El gate de vulnerabilidades del frontend caía a la primera si el registro de npm no contestaba.** `npm audit` pide el endpoint bulk y, si falla, cae al quick… que npm retiró (HTTP 400 «This endpoint is being retired»): un tropiezo del bulk ya no lo rescata nadie. Run 485 del PR: 6m54s colgado y rojo con el MISMO árbol que había pasado en los runs 483 y 484 (3m16s y 4m41s: el registro estuvo degradado la hora entera; en régimen normal tarda segundos). | Job `audit` del run 485 de fuegoservicios-lab/MealfitRD#139. | `scripts/audit-gate.mjs` reintenta la auditoría entera hasta 3 veces (pausas 30 s/60 s) SOLO cuando no hay veredicto; un veredicto con vulnerabilidades no se reintenta; agotados los intentos sigue fail-closed (exit 2); el stdout no-JSON que antes reventaba con SyntaxError es ahora «sin veredicto». Test funcional contra un `npm` falso: `src/__tests__/audit_gate_reintento.test.js` (P1-AUDIT-GATE-REINTENTO). |
