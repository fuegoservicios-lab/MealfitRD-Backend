...
| `get_avg_meal_hour` +8h | **CERRADO** por `P1-AVG-MEAL-HOUR-SIGN · 2026-08-21`, el P-fix dedicado que T5 pedía ([`db_facts.py:311`](../db_facts.py#L311)). Bug PREEXISTENTE (doble `AT TIME ZONE` sobre timestamptz), preservado a propósito durante T5 por su contrato de byte-identidad. La etiqueta «+8h» escondía lo importante: **no es una constante, es `2 × offset`** — con offsets reales por usuario pasó a ser función del país y para España CAMBIA DE SIGNO (RD +8 h, ES −4 h, MX +12 h). Consumidor único: el agente proactivo; un dominicano que desayuna a las 8:00 recibía el nudge a las 17:30. El fix NO es byte-idéntico para DO, deliberadamente. Tests: [`test_p1_country_system_f1.py:2025`](../tests/test_p1_country_system_f1.py#L2025) (reconvertido: anclaba el signo `+`, ahora exige `−`) y [`test_p1_nudge_tz_per_user.py`](../tests/test_p1_nudge_tz_per_user.py) | — |
