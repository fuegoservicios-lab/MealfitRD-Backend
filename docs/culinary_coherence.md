...
| `MEALFIT_CULINARY_JUDGE_GUARD` | **`off`** | `off`/`warn`/`block`. Clamp fail-safe a `off` (no a `warn`) si el valor no es uno de los 3 — a diferencia del contract-guard, esta es una llamada LLM completa por plan, así que el único default seguro ante un valor raro es apagado. Calibrado 2026-08-01: recall re-medido (78%, tras generalizar la rúbrica) queda bajo el floor de 0.80 — **permanece `off`**. Re-calibrado 2026-08-01 post-backfill (3 corridas + mediana): mediana 78%, mismo resultado — **sigue `off`**, la escalada a `warn` espera una corrida futura que cumpla el criterio (Task 14). |
