...
| **Capa 3 — calibración medida** | Script `scripts/calibrate_culinary_judge.py`: recall/FP por clase y por capa contra el golden set (con desglose de FP por tipo) + 1 probe held-out generado en runtime, con llamadas LLM reales (no CI). | **Hecha** (Task 13, 2026-08-01, corregida tras review). Resultado final: capa1 recall 100% + 0 FP (contrato F1 re-confirmado); juez recall 78% + FP 0% (recall bajo el floor de 0.80 — spec §6 NO cumplido en esta corrida); probe held-out out-of-sample **sí cazado** (valida que la regla generalizada de `nombre_no_corresponde` no depende del fixture). |
