...
| Lista de compras | **Bilingüe; el gloss va en el idioma del usuario, con el inglés de respaldo** | [P1-PLAN-LOTE-222 · 2026-09-24] Hasta este lote el gloss era SIEMPRE INGLÉS (P2-I18N-DOC-LISTA-BILINGUE-FALSA lo dejó escrito): `glossShoppingItemName` sólo tenía `name_en`, así que un francés leía «Black beans (Habichuelas rojas)». Ahora cada fila del catálogo trae `names` (`food_names_i18n.py`) y en pt/fr/it gana el nombre propio; el inglés queda de respaldo para una fila sin traducción o un índice publicado antes del lote. Cada línea lleva el gloss Y el nombre canónico español entre paréntesis — «30 g haricots rouges secs (Habichuelas rojas)». El paréntesis no es cortesía: es el identificador con el que resuelve el motor, y el validador descarta la línea que lo pierda. |
...
| Nombres de alimentos y platos (`master_ingredients`, **347 filas**, todas con `name_en`) | **No, jamás** (en el DATO; al LEER sí, desde el lote 222) | Son el **SSOT del motor**. [P1-PLAN-LOTE-222] Lo que se PINTA pasa por el léxico `data/food_names_i18n.json` (349 nombres × 4 idiomas, `food_names_i18n.py`), y lo que el usuario ESCRIBE en su idioma se traduce AL canónico antes de llegar al motor (`canonicos_para_texto`): una alergia a «Strawberry» ya bloquea las «Fresas» (MEDIDO: 32 de 60 declaraciones en otro idioma no bloqueaban). El dato sigue en español. `pantry_names_match` (P1-PANTRY-NAME-RESOLUTION), el guard de coherencia recetas↔lista y el backstop clínico de alergias resuelven por esos nombres exactos. Traducir «Pollo» rompe las tres cosas a la vez, y dos de ellas en silencio. |
...
| Textos que el servidor escribe en español (avisos de calidad del día y del plato, sugerencias de ahorro, «Acción requerida» y bloqueos del Historial, panel de micronutrientes, errores de cupón y de la Nevera) | **Sí**, al pintar | [P1-PLAN-LOTE-222 · 2026-09-24] El título salía traducido y la frase de debajo en español. Lo que lleva cifras viaja además en DATOS (`day_quality_warning_detail`, `brand`/`presentation`/`price_rd`) y el cliente lo recompone; lo fijo se reconoce por coincidencia exacta con un espejo del frontend (`textosDelServidor.js`, `microsCopy.js`) que `test_p1_plan_lote_222.py` compara con el backend (AST), para que una coma cambiada allí no devuelva el español en silencio. |
| Escáner de comida, estimación de macros y plato nuevo al cambiarlo | **Sí** | [P1-PLAN-LOTE-222] El nombre del plato llega en el idioma del usuario (`locale` en la petición); el español original viaja al lado (`meal_name_es`) y los ingredientes siguen en español (descuentan la Nevera). Tras «Cambiar plato»/«Regenerar día» el servidor manda el nombre ya traducido para el aviso y la tarjeta (`display_name`, `meals_display_names`) mientras llega el `_display` completo; esa entrada `_provisional` nunca se guarda (`/restore-local` y `/adopt-guest-plan` la quitan). |
| Plan del invitado (embudo del plan gratis) | **Sí** | [P1-PLAN-LOTE-222] No persiste en la BD, así que ningún disparador de `_display` lo alcanzaba. El cliente manda su copia a `POST /api/plans/guest-display` (`traducir_plan_en_memoria`: mismo motor, sin BD, topes de días/comidas/tamaño y cupo por IP) y fusiona. |
| Texto libre del modelo fuera del plan (lo que el coach recuerda, suplementos del día) | **Sí**, al leer | [P1-PLAN-LOTE-222] `POST /api/i18n/textos` traduce en una llamada lo que la caché del dispositivo no tiene (`useTextosTraducidos`); el gasto va a `llm_usage_events`, nunca a `api_usage`. |
...
«falla si los cinco divergen». **Son diecinueve** (doce hasta el 2026-08-22; [P2-I18N-SEXTO-IDIOMA-ESPEJOS-CONGELADOS · 2026-08-23] sumó los seis que viven fuera de React o en el copy de servidor), y siete no estaban anclados por nada. No se
...
| 19 | `backend/food_names_i18n.py` → `LOCALES` + `backend/data/food_names_i18n.json` (una columna por idioma) | [P1-PLAN-LOTE-222] Una alergia escrita en ese idioma no llega al canónico (el backstop no la ve: antes del lote, 32 de 60 no bloqueaban), el buscador no encuentra lo que el usuario escribe y la Nevera, el escáner y la lista pintan el alimento en español. |
...
[`test_p2_i18n_espejos_sin_ancla.py`](../tests/test_p2_i18n_espejos_sin_ancla.py) cubre 17
espejos con 16 funciones (dos parametrizadas, más el control de que el extractor no esté vacío: los dos addenda del display y las cinco tablas de copy de servidor del coach), y las filas
