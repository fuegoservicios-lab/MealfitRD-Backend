...
Lo que esta medición destapó —`market_check_skipped` en los 57 planes: el paso 3 del compilador (¿el ancla existe en el mercado del país?) solo corría si alguien le pasaba `known_ingredients`, y nadie lo hacía— **quedó cerrado por ARQ27-P1-07 (2026-09-06)**: `build_plan_policy` obtiene `catalog_capability.known_ingredient_names(market_country)` y el compilador anota `market_check_applied` como constancia POSITIVA de que el paso corrió. `market_check_skipped` sobrevive sólo cuando el catálogo es ilegible o está vacío, a propósito: ausente no es lo mismo que «este país no vende nada». *(Actualizado P1-PLAN-FASE-A · 2026-09-11: el párrafo anterior decía abierto lo que ya estaba cerrado, y un doc que reabre un gap cerrado cuesta una sesión.)*
