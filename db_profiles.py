...
_DEFAULT_LLM_PRICING_MICROS_PER_M: Dict[str, Dict[str, int]] = {
    ...
    #
    # [P1-REVIEWER-TIER-MODELS · 2026-07-31] Re-consultado tras el recorte de OpenAI:
    #   luna:  $0.20 in / $1.20 out  (-80% vs 2026-07-26: era $1.00/$6.00)
    #   terra: $2.00 in / $12.00 out (-20% vs 2026-07-26: era $2.50/$15.00)
    # `cached` mantiene el ratio 10% del input (convención OpenAI observada en luna).
    #
    # ⚠️ OJO con el `cached`: 10× de descuento vs entrada normal, y el day-gen viene con ~42%
    # de aciertos de caché medidos. Ignorarlo sobreestima el costo de Luna en su entrada.
    #
    # NO cubierto: la tarifa de ESCRITURA de caché. `usage_metadata` no la separa, así que esas
    # escrituras se cobran aquí como entrada normal — costo real ligeramente subestimado en la
    # primera llamada de cada prompt nuevo.
    #
    # El tier PRIORITY cuesta ~2× y NO se usa; si algún día se activa, duplicar estas filas con
    # otro sufijo de modelo.
