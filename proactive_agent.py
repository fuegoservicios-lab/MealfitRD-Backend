...
# [P1-PLAN-LOTE-222 · 2026-09-24] LA HORA DE CADA AVISO LA ELIGE LA PERSONA.
#
...
# gana siempre. tooltip-anchor: P1-PLAN-LOTE-222-HORA-ELEGIDA
...
def minuto_del_dia(hora: float) -> int:
    ...
    teléfono redondeaba («6:50»): el mismo aviso con dos horas. [P1-PLAN-LOTE-222] Una sola función para las dos vías
...
# [P1-PLAN-LOTE-222 · 2026-09-24] Cada cuántos minutos corre el cron (app.py lo registra con este mismo número).
#
...
def run_proactive_checks():
    ...
    for s in sessions:
        ...
        candidatas = []
        # [P1-PLAN-LOTE-222] El perfil se lee UNA vez por usuario y tick: la rama de las comidas lo necesita antes
        ...
        if now_ast.hour == HORA_DEL_RESUMEN:
            # [P1-PLAN-LOTE-222] Con el cron cada 15 min, la hora 23 tiene cuatro ticks: el resumen sale en el primero
            ...
            try:
                ...
                logger.warning(f"[P1-PLAN-LOTE-222] perfil de {user_id} ilegible ({e}); avisos a las horas normales.")
            ...
            for _orden, (meal, def_hour) in enumerate(defaults.items()):
                # [P1-PLAN-LOTE-222] El interruptor de ESTA comida (Configuración → Recordatorios de comida).
                ...
                # [P1-PLAN-LOTE-222] «Su hora» es el último tick ANTES de que suene el teléfono (ver
        ...
        try:
            ...
            # [P1-PLAN-LOTE-222] el que ya leyó la rama de las comidas; el Resumen del día lo lee aquí
