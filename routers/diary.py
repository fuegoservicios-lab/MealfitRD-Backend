...
class EstimateMacrosRequest(BaseModel):
    ...
    # [P1-PLAN-LOTE-224 · 2026-09-24] El idioma de la pantalla: el nombre y la porción vuelven en él. Antes el modelo
...
_ESTIMATE_SYSTEM_PROMPT = (
    ...
    '{"name": "nombre corto del plato (≤60 caracteres, en el idioma que se indica abajo)", '
...
class ConsumedMealRequest(BaseModel):
    ...
    # [P1-PLAN-LOTE-223 · 2026-09-24] «El plato lleva esto» y «sácalo de mi Nevera» dejan de ser la misma casilla.
...
async def api_diary_upload(
    ...
    # [P1-PLAN-LOTE-224 · 2026-09-24] El idioma que la pantalla está mostrando (`getLocale()`): el nombre del plato y el
    ...
    try:
        ...
        # [P1-PLAN-LOTE-224 · 2026-09-24] Un tester con la app en inglés leía «Un tazón de avena cocida con leche» en
        ...
        return {
            ...
            "meal_name": meal_name,
            # [P1-PLAN-LOTE-224] El nombre que decidió el motor, en español (igual a `meal_name` si no se tradujo).
...
async def _locale_del_escaner(locale_form, verified_user_id) -> str:
    """[P1-PLAN-LOTE-224] El idioma que manda la pantalla; si no llega (cliente anterior), el del perfil; si no, el base."""
    ...
    if verified_user_id:
        try:
            prof = await asyncio.to_thread(get_user_profile, verified_user_id)
            loc = locale_soportado((prof or {}).get("locale"))
        except Exception:
            loc = None
    return loc or LOCALE_BASE


async def _nombres_para_mostrar(meal_name: str, items: list, locale_form, verified_user_id):
    """[P1-PLAN-LOTE-224 · 2026-09-24] (`meal_name` traducido, `items` con `display_name`) para el idioma de la pantalla.
    ...
    from traduccion_para_mostrar import traducir_para_mostrar
    from food_names_i18n import nombre_para
    nombres = [str((it or {}).get("name") or "") for it in (items or [])]
    trad = await traducir_para_mostrar([meal_name, *nombres], loc, user_id=verified_user_id,
                                       node="vision_scan_display_i18n")
    out = []
    for i, it in enumerate(items or []):
        it2 = dict(it or {})
        propio = (trad[i + 1] if trad else "") or nombre_para(it2.get("name") or "", loc) or ""
        if propio and propio != it2.get("name"):
...
def api_log_consumed_meal(
    ...
    try:
        ...
        return _persist_consumed_meal(
            ...
            source="photo",
            # [P1-PLAN-LOTE-223] solo un `False` explícito apaga la resta; ausente = la conducta de siempre
...
async def api_estimate_macros(
    ...
    # [P1-PLAN-LOTE-224] `name` y `portion_note` en el idioma de la pantalla (o del perfil); el resto del JSON no cambia.
    try:
        ...
        if not _loc:
            _prof = await asyncio.to_thread(get_user_profile, verified_user_id)
            _loc = locale_soportado((_prof or {}).get("locale"))
        _idioma = _COACH_LANGUAGE_NAMES.get(_loc or "")
    except Exception:
        _idioma = None
    human += (f"\nEscribe 'name' y 'portion_note' en {_idioma}." if _idioma
              else "\nEscribe 'name' y 'portion_note' en español.")
