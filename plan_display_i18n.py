...
def _display_ya_usable(meal: dict, locale: Optional[str]) -> bool:
    ...
    if not isinstance(entrada, dict):
        return False
    # [P1-PLAN-LOTE-224 · 2026-09-24] Una entrada `_provisional` (sólo el nombre de un plato recién cambiado, puesta
...
# [P1-PLAN-LOTE-224 · 2026-09-24] Topes del plan de un INVITADO que se traduce en memoria (sin BD). Es la única
...
def traducir_plan_en_memoria(plan_data: dict, locale: str, *, user_id: Optional[str] = None) -> dict:
    """[P1-PLAN-LOTE-224 · 2026-09-24] La traducción de un plan que NO está en la base de datos: el del invitado.
    ...
    try:
        ...
        while pendientes and presupuesto > 0:
            ...
            try:
                ...
                logger.warning(f"[P1-PLAN-LOTE-224] traducción del plan invitado falló ({locale}): {e!r}")
                razon = "llm_exception"
                if presupuesto > 0:
        ...
        logger.warning(f"[P1-PLAN-LOTE-224] traducir_plan_en_memoria excepción (fail-open) {locale}: {e!r}")
