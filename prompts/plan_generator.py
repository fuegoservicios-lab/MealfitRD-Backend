...
2.5. TRANSFORMA LOS STAPLES EN PLATOS CRIOLLOS APETECIBLES [P2-CREATIVITY-TRANSFORM · 2026-06-29]: NO sirvas el staple "crudo/simple" por defecto ni "proteína a la plancha + arroz blanco" en cada comida. Conviértelo en una preparación criolla (manteniendo CADA componente desglosado en `ingredients` para que la lista lo costee): harina → panqueques/bollos/arepas/tortillas; avena → panqueques de avena/overnight; yuca → bollos de yuca/arepitas/casabe; plátano → mofongo/mangú/tostones; huevo → tortilla/revoltillo. Aplica ESPECIALMENTE a merienda y cena. La creatividad es en la PREPARACIÓN, NUNCA en inventar alimentos fuera del catálogo verificado.
...
6. VARIEDAD (INTRA Y EXTRA DÍA): Si hay disponibilidad, no repitas la misma proteína en el desayuno, almuerzo y cena del mismo día. Usa diferentes fuentes secundarias (ej: huevos, quesos) para el desayuno y meriendas SI ESTÁN DISPONIBLES EN LA DESPENSA actual. IMPORTANTE: En el modo rotación, NUNCA inventes ingredientes si no están en la despensa, es preferible repetir un ingrediente que usar uno inexistente. No repitas platos exactos recientes.
...
def build_sleep_stress_context(form_data: dict) -> str:
    ...
    _STRESS_HINTS = {
        ...
        "Muy Alto": (
            ...
            "Sesga hacia magnesio + omega-3 + carbohidrato complejo rico en "
            "fibra. Tono empático y validante en los `desc`."
        ),
    }
