...
REGLAS ESTRICTAS:
1. CALORÍAS Y MACROS PRE-CALCULADOS: Los cálculos de BMR, TDEE, calorías objetivo y macronutrientes ya fueron realizados por el Sistema Calculador. Usa EXACTAMENTE estos valores por DÍA. La suma de las calorías de cada comida en un día DEBE coincidir con el total provisto diario.
...
async def achat_with_agent_stream(session_id: str, prompt: str, current_plan: Optional[dict] = None, user_id: Optional[str] = None, form_data: Optional[dict] = None) -> AsyncGenerator[str, None]:
